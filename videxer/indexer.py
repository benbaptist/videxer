...
def _strip_subtitle_data_from_hierarchy(items: List[Dict]) -> None:
    """Recursively strip subtitle text/timestamp data from hierarchical items, keeping only file references.
    
    Modifies items in place.
    """
    for item in items:
        if item.get('type') == 'media' and 'subtitles' in item:
            # Keep only file references, remove parsed text/timestamps data
            subtitle_files = []
...
def build_index(root: Path, generate_thumbnails: bool = False, generate_motion_thumbnails: bool = False, generate_transcodes: bool = False) -> Dict:
  ...
  # Build search indexes, then strip bulky subtitle text/timestamp data from items
  base_search_index = _build_base_search_index(processed_items)
  subtitle_index = _build_subtitle_index(processed_items)

  # Strip subtitle text/timestamp data from hierarchical structure (keep only file references)
...
def write_index_files(root: Path, html_path: Optional[Path] = None, json_path: Optional[Path] = None, generate_thumbnails: bool = False, generate_motion_thumbnails: bool = False, generate_transcodes: bool = False) -> None:
  ...
  # Compile template and write HTML + assets
  from .compiler import build_html
  assets_dir = root / ".videxer" / "assets"
  assets_dir.mkdir(parents=True, exist_ok=True)
  html_content, css_content, js_content = build_html(version('videxer'), assets_dir)
  ...
  log.debug(f"Wrote assets: {assets_dir}")
